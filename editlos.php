...
<?php include "sidebar.php";?>	  
	  <div class="content-wrapper">
          <h3 class="page-heading mb-4">Edit Data Los</h3>
          <div class="row mb-2">
            <div class="col-lg-10">
              <div class="card">
                <div class="card-body">
				<?php 
$ubah = mysqli_query ($koneksi, "select * from los where id_los ='$ud1'");
...
if (isset($_POST['edt'])) {
	...
	$cek = mysqli_query($koneksi, "select * from los where id_los='$data[0]'"); //cek data yg mau diedit
	
  if (mysqli_num_rows($cek) !=0) {
		$edit = mysqli_query($koneksi, "UPDATE `los` set  `nama_los`='$nama',`id_penyewa`='$np',`id_blok`='$nb',`ukuran`='$ukrn',`harga_sewa`='$hrs' where `id_los`='$ud1'") 
		or die (mysqli_error());
		
				if($edit) {
			echo '<script type="text/javascript">alert("Data Berhasil disimpan") </script>';
			echo '<meta http-equiv="refresh" content="0; url=./laplos.php" >'; //coding refresh
...
?>
                ...
                <div class="form-group"   >
                	<label class="col-sm-3 control-label">ID Los</label>
                    <div class="col-sm-2">
                    	<input type="text" name="i" class="form-control" placeholder="ID Penyewa" value="<?php echo $data[0];?>" readonly>
                       </div>
					   <label class="col-sm-3 control-label">Nama Los</label>
                    <div class="col-sm-6">
                    	<input type="text" name="nama" class="form-control" placeholder="Nama Toko" value="<?php echo $data[1];?>">
                       </div>
					   <label class="col-sm-3 control-label">Penyewa</label>
					   <div class="col-sm-5">
                    <select class="form-control" span="label label-success" name="np">
		<option target="#option1" value="<?php echo $data[2];?> "><?php echo $data[2];?> </option>
		<?php
		...
		<option target="#option2" value="<?php echo $data[3];?>"><?php echo $data[3];?> </option>
		<?php
		...
		?>
                    ...
                    <div class="col-sm-3">
                       ...
                       <a href="laplos.php" type="button" class="btn btn-danger mr-2"><span class="fa fa-remove"> Batal</span></a>
                       </div>
                       </div>
               </form>
                </div>
